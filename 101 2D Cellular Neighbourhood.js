...
function getNeighbourhood(type, matrix, [x, y]) {
   if (matrix[x] === undefined || matrix[x][y] === undefined) {
      return [];
   }
   return (type === 'moore' ? [[x - 1, y - 1], [x - 1, y], [x - 1, y + 1], [x, y - 1], [x, y + 1], [x + 1, y - 1], [x + 1, y], [x + 1, y + 1]] :
      [[x - 1, y], [x, y - 1], [x, y + 1], [x + 1, y]]).map(([x, y]) => matrix[x]?.[y]).filter(el => el !== undefined);
}